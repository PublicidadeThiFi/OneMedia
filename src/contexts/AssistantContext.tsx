...
interface AssistantContextValue {
  ...
  importSession: AssistantImportSession | null;
...
export function AssistantProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const stored = readAssistantLocalState<{
      messages?: AssistantMessage[];
      suggestedPrompts?: string[];
      proactivePrompts?: string[];
      providerName?: string;
  ...
  useEffect(() => {
    writeAssistantLocalState(getAssistantStorageKey(userId), {
  ...
  useEffect(() => {
    ...
    if (readAssistantLocalState<boolean>(dailyShownKey)) return;
      ...
      try {
        ...
        if (messageContent) {
          ...
          const pushMsg: AssistantMessage = {
            ...
            operationalExceptions: briefing.exceptions,
          };
          setMessages((prev) => [...prev, pushMsg]);
          setHasUnreadBriefing(true);
          hasUnreadBriefingRef.current = true;
